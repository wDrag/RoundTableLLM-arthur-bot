...
fastify.post("/api/chat", async (request, reply) => {
    ...
    if (!parsed.success) {
        request.log.warn(
            { requestId: request.id, issues: parsed.error.issues },
    ...
    const normalizedContext = `Discord context: guildId=${
        payload.context?.guildId ?? "unknown"
    }, channelId=${payload.context?.channelId ?? "unknown"}`;
    ...
    request.log.info(
        { requestId: context.requestId, plan },
        "Agent routing plan"
    );

    const { outputs, timings } = await runAgents(plan, context, request.log);
    request.log.info(
        { requestId: context.requestId, timings },
        "Agent timings"
    );
